...
def dartboard():
    return [L+str(i) for L in 'TDS' for i in range(1,21)] + ['SB','DB']

def score(section):
    if section[0:3] == 'OFF':
        return 0
    multiplier = {'T':3,'D':2,'S':1}    
    mult, num = section[0], section[1:]
    return (25 if section == 'SB' 
    else 50 if section == 'DB' 
    else int(num) * multiplier[mult])

def double_out(total):
    board = dartboard()
    combs = (itertools.chain(
        itertools.product(board),
        itertools.product(board,repeat=2),
        itertools.product(board,repeat=3)))
    end_double = list(filter(lambda x: x[-1][0] == 'D',combs))
    total_score_match = [list(comb) for comb in end_double 
                                    if sum(map(score,comb))==total]
    return None if not total_score_match else min(total_score_match,key=len)

# def double_out(total):
#     db = dartboard()
#     res = set()
#     def double_out(total,accumulator=None):
#         if not accumulator: accumulator = list()
#         for section in db:
#             remainder = total - score(section)
#             print(remainder)
#             if not remainder: 
#                 res.add(tuple([section]+accumulator))
#             elif remainder > 0:
#                 double_out(remainder,accumulator+[section])
#         return res
#     return double_out(total)
...
ring_misses = {
    ...
    'D': {'OFF':0.5,'S':0.5},
    'S': {'D':0.5,'T':0.5},
    'SB':combine_dict({'DB':0.25},{'S'+str(s):0.75/20 for s in range(1,21)}),
    'DB':combine_dict({'SB':1/3}, {'S'+str(s):2/3/20 for s in range(1,21)})
    }

def outcome(target, miss):
    "Return a probability distribution of [(target, probability)] pairs."
    if target in ['SB','DB']:
        ring, section = target, 'B'
    else:
        ring, section = target[0], int(target[1:])
    updated_miss = min(1,
                      (miss * 3.0 if ring == 'DB' 
                                  else miss / 5.0 if ring == 'S' 
                                  else miss)
                      )
    r_miss = {loc:updated_miss*prob for loc,prob in ring_misses[ring].items()}
    r_hit = {ring: 1 - updated_miss}
    s_miss = {s:miss/len(adjacent_targets(section)) 
        for s in adjacent_targets(section)}
    s_hit = {section: 1 - miss}
    rings = combine_dict(r_hit,r_miss)
    sections = combine_dict(s_hit,s_miss)
    return joint_probs(target,rings,sections)

def joint_probs(target,ring_p,section_p):
    jointp = {}
    if target in ['SB','DB']:
        for rp in ring_p:
            for sp in section_p:
                if rp == sp:
                    jointp[sp] = (ring_p[rp] + section_p[sp] - 
                                  ring_p[rp] * section_p[sp] *
                                  20)
                elif rp[1:] == 'B' and sp == 'B':
                    jointp[rp] = ring_p[rp]*section_p[sp]
    else:
        for rp in ring_p:
            for sp in section_p:
                jointp[rp+str(sp)] = ring_p[rp]*section_p[sp]
    return jointp

def adjacent_targets(section):
    if section == 'B':
        return ['S'+str(s) for s in range(1,21)]
    pos = sections.index(section)
    left = sections[pos-1]
    right = sections[min(pos+1,(pos + 1 - len(sections)))]
    return([left,right])

def best_target(miss):
    "Return the target that maximizes the expected score."
    return max([(dart,expected_value(outcome(dart,miss)))for  dart in dartboard()]
        ,key = lambda x: x[1])[0]

def expected_value(dist):
    return sum(score(dart)*prob for dart,prob in dist.items())
