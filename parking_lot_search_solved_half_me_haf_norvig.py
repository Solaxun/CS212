...
def update_grid(new_locs,car,grid):
	...
	for nl in new_locs:
		...
		if nl < min(old_locs):
			...
			action = [car[0],nl - min(old_locs)]
		elif nl > max(old_locs):
			grid[car[0]] = locs(nl - d * (len(old_locs)-1),len(old_locs),d)
			action = [car[0],nl - max(old_locs)]
		new_grids.append((tuple(grid.items()),tuple(action)))
	return new_grids
...
def successors(state):
	...
	for (c, sqs) in state:
		if c not in '|@': # Walls and goals can't move
			...
			new_locs = legal_moves((c,sqs),state)
			if new_locs:
				# print(c,new_locs)
				new_grid = update_grid(tuple(new_locs),(c,sqs),state)
				for g in new_grid:
					s,action = g
					# print(action)
					# print('*'*40)
...
print(show(puzzle4,N=8,pretty=True))
# print(path_actions(solve_parking_puzzle(puzzle4,N)))
test_parking()

# for state,action in (successors(puzzle4).items()):
# 	print(show(state,pretty=True),'-->',action)
