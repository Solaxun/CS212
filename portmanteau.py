...
def score(portmanteau):
	start,mid,end = portmanteau
	l = len(start+mid+end)
	actual = map(len,[start,mid,end])
	ideal = (l/4,l/2,l/4)
	return reduce(lambda x,y: x-y,[abs(i-a) for i,a in zip(ideal,actual)],l)

def replace_list(lst,old):
	return [item for item in lst if item != old]

def natalie(words,res=None):
	if res == None:
		res = set()
	if not words:
		return None
	for startword in words:
		for i in range(len(startword)-1):
			s = startword[i+1:]
			for endword in replace_list(words,startword):
				for j in range(len(endword)-1):
					e = endword[0:j+1]
					if s == e:
						start = startword[0:i+1]
						mid = e
						end = endword[j+1:]
						# print(start,mid,end)
						res.add((start,mid,end))
	return ''.join(max(res,key=score)) if res else None
