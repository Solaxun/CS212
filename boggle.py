...
def boggle_words_2(board,start=None,minlength=3,pre=None,explored=None,res=set()):
    # print(start,pre,explored)
    if len(pre) >= minlength and pre in WORDS:
        res.add(pre)
    if pre in PREFIXES:
        for neighbor in neighbors(start,size(board)):
            if neighbor not in explored and board[neighbor] is not BORDER:
                explored.add(neighbor)
                boggle_words_2(board,neighbor,minlength,pre+board[neighbor],explored,res)
    explored.remove(start) #if bactracking, remove from explored set
    return res

def boggle_words(board,minlength=3):
    final = set()
    start,end = size(board), len(board) - size(board) 
    for begin in range(start,end):
        pre = board[begin]
        explored = {begin}
        res = boggle_words_2(board,begin,minlength,pre,explored,res=set())
        final = final | res
    return final
...
# ||||||""".strip()
    ...
    assert neighbors(20, 6) == set([13, 14, 15, 19, 21, 25, 26, 27])
...
def neighbors(i, N):
    """modified to return set so i can remove successor states if explored already"""
    return set([i-N-1, i-N, i-N+1, i-1, i+1, i+N-1, i+N, i+N+1])
