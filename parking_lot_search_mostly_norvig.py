...
def update_grid(new_locs,car,grid):
	...
	car, old_locs = car
	d = old_locs[1] - old_locs[0]
	gridmap = dict(grid)
	new_grids = []
	for nl in new_locs:
		grid = deepcopy(gridmap)
		if nl < min(old_locs):
			grid[car] = locs(nl,len(old_locs),d)
			action = [car,nl - min(old_locs)]
		elif nl > max(old_locs):
			grid[car] = locs(nl - d * (len(old_locs)-1),len(old_locs),d)
			action = [car,nl - max(old_locs)]
		new_grids.append((tuple(grid.items()),action))
	return new_grids
##########################################
...
def legal_moves(car,grid):
	# N = int((max(dict(grid)['|'])+1) ** 0.5)
	car, locs = car[0], car[1]
	full = set(s for (c, locs) in grid for s in locs if c != '@')
	diff = locs[1] - locs[0]
	# Either move the max of sqs up, or the min of sqs down
	# new_locs = []
	results = {}
	for (d, start) in [(diff, max(locs)), (-diff, min(locs))]:
		for i in range(1, N-2):
			#new locations not new car position
			s = start + d*i
			if s in full:
				break # Stop when you hit something
			#MCW: literally the one thing that makes this not work is changing
			#the tuple (car, d*i) to list [car, d*i], which was my problem
			results[update(grid,car,tuple(q+d*i for q in locs))]=(car,d*i)
	return results

def successors(state):      
	cars = [car for car in state if car[0] not in '@|']
	lmoves = lambda car: legal_moves(car,state)
	return {k:v for d in map(lmoves,cars) for k,v in d.items()}
